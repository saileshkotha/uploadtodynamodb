...
function uploadData(items, lastKey, curr_batch) {
    ...
    try {
        if (item["Out / Inpatient"]) {
            record.in_out_patient = cleanArray(item["Out / Inpatient"]);
        }
    } catch (e) {
    }
    ...
    try {
        if (item["Treatment Programs"]) {
            record.treatment_programs = cleanArray(item["Treatment Programs"]);
        }
    } catch (e) {
    }
    ...
    try {
        ...
        if (qualif["Board Certification"]) {
            record.qualifications.board_certifications = qualif["Board Certification"]
        }
        try {
    ...
    try {
        ...
        if(item.Finances.slice(3)){
            record.finance.network_provider = cleanArray(item.Finances.slice(3))
        }
